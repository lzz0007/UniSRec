...
class DBN(nn.Module):
    def __init__(self, num_features, num_groups=32, num_channels=0, dim=2, eps=1e-5, momentum=0.1, affine=True, mode=0,
        ...
        if self.affine:
            ...
            self.white_linear = nn.Linear(num_features, 300)
        else:
...
class UniSRecWhiten(SASRec):
    def __init__(self, config, dataset):
        ...
        # self.white_linear = MLPLayers([768, self.hidden_size, self.hidden_size], 0.2, 'relu')
    ...
    def calculate_loss(self, interaction):
        ...
        if self.engine == 'bn':
            test_item_emb = self.white_linear(self.bn(self.plm_embedding.weight))
        else:
